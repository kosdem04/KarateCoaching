...
from src.requests.tournaments import TournamentRequest
...
@router.get("/",
            tags=["Турниры"],
            summary="Просмотр всех турниров",
            response_model=list[tournaments_schemas.TournamentModel]
         )
async def get_user_tournaments(session: SessionDep,
                               user_id: AuthUserDep):
    tournaments = await TournamentRequest.get_user_tournaments(session, user_id)
    return tournaments
